    ...
    $.openPopupLayer = function(settings) {
        ...
        if (!checkIfItExists(settings.name)) {
            settings = jQuery.extend({
                width: "auto",
                height: "auto",
                parameters: {decorator:"popups",confirm:"true"},
                target: "",
                success: function() {try{$(".pop_content :input[type !='hidden']:first").focus()}catch(e){}},
                error: function() {},
                beforeClose: function() {var nc_date=$(".nc_date");if(nc_date.length > 0)nc_date.datepicker('hide') },
                afterClose: function() {},
                reloadSuccess: null,
                cache: false
            }, settings);
    ...
    function setScreenLockerSize() {
        if (popupLayerScreenLocker) {
            ...
            if (document.body.clientWidth < 1000) {
        			$('#popupLayerScreenLocker').width("1000px");
        		}else {
    ...
    function setPopupLayersPosition(popupElement, animate) {
        if (popupElement) {
            ...
            if (popupElement.height() < $(window).height()) {
                var topPosition = pos_top + ($(window).height() - popupElement.height()) / 2;
            } else {
                var topPosition = pos_top + 5;
            }
            var positions = {
