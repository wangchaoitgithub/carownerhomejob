...
/*!
					...
					*/
"selected":[]}},i;for(i in this._model.data){if(this._model.data.hasOwnProperty(i)){if(i!=="#"){if(this._model.data[i].state.opened){state.core.open.push(i)}if(this._model.data[i].state.selected){state.core.selected.push(i)}}}}return state},set_state:function(state,callback){if(state){if(state.core){var res,n,t,_this;if(state.core.open){if(!$.isArray(state.core.open)){delete state.core.open;this.set_state(state,callback);return false}res=true;n=false;t=this;$.each(state.core.open.concat([]),function(i,v){n=t.get_node(v);if(n){if(t.is_loaded(v)){if(t.is_closed(v)){t.open_node(v,false,0)}if(state&&state.core&&state.core.open){$.vakata.array_remove_item(state.core.open,v)}}else{if(!t.is_loading(v)){t.open_node(v,$.proxy(function(o,s){if(!s&&state&&state.core&&state.core.open){$.vakata.array_remove_item(state.core.open,o.id)}this.set_state(state,callback)},t),0)}res=false}}});if(res){delete state.core.open;this.set_state(state,callback)}return false}if(state.core.scroll){if(state.core.scroll&&state.core.scroll.left!==undefined){this.element.scrollLeft(state.core.scroll.left)}if(state.core.scroll&&state.core.scroll.top!==undefined){this.element.scrollTop(state.core.scroll.top)}delete state.core.scroll;this.set_state(state,callback);return false;
/*!
