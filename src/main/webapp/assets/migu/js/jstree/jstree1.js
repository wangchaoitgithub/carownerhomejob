 ...
 */
	...
	$.jstree.core.prototype = {
		...
		bind : function () {
			this.element
				...
				.on("click.jstree", ".jstree-ocl", $.proxy(function (e) {
						//console.log("jstree-ocl click.jstree");
						//console.log(e);
						if( this.is_closed(e.currentTarget) ) { 
							//this.close_all('0');
							//this.open_node('#');
							var nodes = this.get_children_dom('0');
							if( nodes.length > 0 ) {
								for( var i=0;i<nodes.length;i++ ) { 
									if( this.is_open( nodes[i] ) )
										this.close_node( nodes[i] );
								}
							}
						}
						this.toggle_node(e.target);
		...
		_append_json_data : function (dom, data, cb, force_processing) {
          ...
          console.log("dat.length = " + dat.length + ", dat[0].id = " + dat[0].id + ", dat[0].parent = " + dat[0].parent);
		...
		close_node : function (obj, animation) {  
			...
			if(d.length) {
				if(!animation) {
					...
					console.log("jstree-children remove");
				}
				else {
					d
		...
		create_node : function (par, node, pos, callback, is_loaded) {
			...
			if(node.text == "")  { node.text = "新建分类"; }
			//alert(node.text);
			if (!this.get_string('Newnode')) {
				node.text = "新建分类";
			}
			var tmp, dpc, i, j;
